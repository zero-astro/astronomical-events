...
### Phase 5: Polish & Monitoring — COMPLETED ✅
**Goal:** Production-ready with monitoring and user feedback.

**Completion Date:** May 2, 2026

**Tasks Completed:**
1. [x] Web dashboard (FastAPI app showing upcoming events) — `dashboard.py` fully implemented with HTML UI, API endpoints (`/api/events`, `/api/stats`, `/api/events/today`), CLI entry point via `main.py dashboard`
2. [ ] Manual notification trigger via Telegram command (`/notify now`) — NOT DONE (no Telegram bot client; system uses stdout JSON routing instead)
3. [ ] Config update via Telegram commands (`/set_location 43.1,-2.9`) — NOT DONE (same reason as #2)
4. [x] Performance: batch fetch thumbnails, cache event pages — `cache.py` implemented with file-based TTL cache
...
| Phase 5: Polish & Monitoring | 2-3 days | Phase 4 | ✅ Done (May 2) |
| Phase 6: Rich Metadata Extraction & Structured Output | 1-2 days | Phase 5 | ✅ Done (May 2) |
| Phase 7: Mastodon Footer Preservation & Separate Posting Paths | <1 day | Phase 6 | ✅ Done (May 2)

**Total estimated:** 10-16 development days (part-time)

---

### Phase 6: Rich Metadata Extraction & Structured Output — COMPLETED ✅
...
**Tasks Completed:**
1. [x] Add 3 new columns to `events` table (`rich_description_en`, `viewing_info_en`, `event_details_json`) + migration logic in `db_manager.py`
2. [x] Enhance `page_scraper.py` to extract rich metadata from event pages (meteor showers, lunar events, conjunctions) — `_extract_event_details()` implemented
3. [x] Update `main.py` pipeline: RSS fetch → enrichment phase (batch scrape with 1s rate limit) → store enriched data
4. [x] Extend translation pipeline (`translator.py`) for new fields + add columns to `translations` table (`translated_rich_description`, `translated_viewing_info`)
5. [x] Upgrade Mastodon format to use structured card sections ("Zer da?", "Non ikusi?", etc.)
6. [x] Test end-to-end: fetch → enrich → translate → post, verify JSON schema compatibility across channels
7. [x] Update timeline estimate and mark Phase 6 as complete
