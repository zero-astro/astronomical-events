...
Usage:
    ...
    python main.py translate-rich             Re-translate rich_description_en and viewing_info_en (Phase 6)
"""
...
from notification import send_notifications, post_individual_events, post_digest
...
# Lazy imports to avoid unnecessary dependencies (FastAPI etc.)
...
def _import_translate():
    ...
    return get_provider_config, PROVIDERS
...
def cmd_fetch(config):
    ...
    for item in items:
        ...
        inserted = db.insert_event(
            ...
            rich_description_en="",   # Phase 6: populated by cmd_process
            viewing_info_en="",       # Phase 6: populated by cmd_process
            event_details_json="",    # Phase 6: populated by cmd_process
        )
...
def cmd_process(config):
    ...
    for item in items:
        ...
        # Scrape page for thumbnail, visibility level, and rich metadata (Phase 2 + Phase 6)
        thumbnail_url = None
            ...
            try:
                ...
                if html:
                    ...
                    if page_data and page_data.is_visible:
                        ...
                        scraped_count += 1
                    # Phase 6: Extract rich metadata from any successful scrape
                    if page_data:
                        rich_description_en = page_data.rich_description_en or ""
                        viewing_info_en = page_data.viewing_info_en or ""
                        event_details_json = page_data.event_details_json or ""
            except Exception as e:
                logger.warning(f"Failed to scrape page for {item.get('news_id')}: {e}")

        # Insert/update event with full metadata (Phase 2 + Phase 6)
        inserted = db.insert_event(
            news_id=item["news_id"],
            title=item["title"],
            event_date=event_date,
            ...
            event_page_url=item.get("link"),
            rich_description_en=rich_description_en,
            viewing_info_en=viewing_info_en,
    ...
    # Count Phase 6 enrichment
    enriched = db.conn.execute(
        "SELECT COUNT(*) FROM events WHERE rich_description_en != '' AND event_details_json != ''"
    ).fetchone()[0]
    ...
    logger.info(f"   Phase 6 enriched:{enriched}")
    logger.info(f"   Total in DB:     {db.count_events()}")
...
def cmd_notify_now(config):
    ...
    # Phase 3: Agent-native notification output
...
def cmd_translate_rich(config):
    """Re-translate rich_description_en and viewing_info_en for all events (Phase 6)."""
    setup_logging()
    logger.info("=" * 60)
    logger.info("Astronomical Events - Translate Rich Metadata (Phase 6)")
    logger.info("=" * 60)
    ...
    # Get all events that have rich_description_en or viewing_info_en populated
    cursor = db.conn.cursor()
    cursor.execute("""
        SELECT * FROM events 
        WHERE (rich_description_en != '' OR viewing_info_en != '')
          AND is_notified = 0
        ORDER BY event_date ASC
    """)
    rows = cursor.fetchall()
    events = [db._row_to_event(row) for row in rows]

    if not events:
        logger.info("No enriched events found. Run 'process' first to populate rich metadata.")
        db.close()
        return

    logger.info(f"Found {len(events)} event(s) with rich metadata")

    total_translated = 0
    total_failed = 0
    batch_size = 3  # Small batches for longer text fields
    ...
    for lang in target_langs:
        ...
        for i in range(0, len(events), batch_size):
            ...
            # Translate rich_description_en fields
            enriched = [e for e in batch if e.rich_description_en]
            if enriched:
                try:
                    texts = [e.rich_description_en for e in enriched]
                    translated = translate_batch(texts, lang, provider_config)
                    for j, evt in enumerate(enriched):
                        cursor.execute(
                            "UPDATE translations SET translated_rich_description=? WHERE news_id=? AND target_lang=?",
                            (translated[j], evt.news_id, lang)
                        )
                        total_translated += 1
                except Exception as e:
                    logger.error(f"Rich desc translation failed ({lang}, batch {i}): {e}")
                    total_failed += len(enriched)
            
            # Translate viewing_info_en fields
            with_viewing = [e for e in batch if e.viewing_info_en]
            if with_viewing:
                try:
                    texts = [e.viewing_info_en for e in with_viewing]
                    translated = translate_batch(texts, lang, provider_config)
                    for j, evt in enumerate(with_viewing):
                        cursor.execute(
                            "UPDATE translations SET translated_viewing_info=? WHERE news_id=? AND target_lang=?",
                            (translated[j], evt.news_id, lang)
                        )
                        total_translated += 1
                except Exception as e:
                    logger.error(f"Viewing info translation failed ({lang}, batch {i}): {e}")
                    total_failed += len(with_viewing)
    ...
    db.close()
    logger.info("-" * 40)
    logger.info(f"Rich translation complete! {total_translated} fields translated, {total_failed} failed")
...
def cmd_post_event(config):
    """Post individual high-priority events (P1-P3) to Mastodon/Telegram."""
    setup_logging()
    stats = post_individual_events(config)
    logger.info(f"Individual events posted - Immediate: {stats['sent_immediate']}, Batch: {stats['sent_batch']}, Failed: {stats['failed']}")


def cmd_post_digest(config):
    """Post daily digest (P4-P5) to Mastodon/Telegram."""
    setup_logging()
    stats = post_digest(config)
    logger.info(f"Digest posted - Events: {stats['sent_digest']}, Failed: {stats['failed']}")
...
def main():
    ...
    cmd_dashboard = None
    get_provider_config, PROVIDERS = None, None

    if command == "dashboard":
        cmd_dashboard = _import_dashboard()
    elif command in ("translate", "translate-all", "translate-rich"):
        get_provider_config, PROVIDERS = _import_translate()
    ...
    commands = {
        ...
        "health": cmd_health,
        "dashboard": lambda cfg: _import_dashboard()(cfg),
        "history": cmd_history,
        "translate": cmd_translate,
        "translate-all": cmd_translate_all,
        "translate-rich": cmd_translate_rich,
