...
def classify_event(title: str, description_text: str = "") -> Classification:
    ...
    # P5: Moon orbital events (apogee, perigee, aphelion, new/full moon phases)
    if _is_moon_orbital_event(title_lower):
        classification.event_type = "moon_phase"
        classification.priority = 5
        return classification

    # P4: Deep-sky objects (Messier catalogs, NGC, well-placed events)
    if _is_deep_sky_object(title_lower):
        classification.event_type = "deep_sky"
        classification.priority = 4
        return classification
...
def _is_moon_orbital_event(title: str) -> bool:
    """Check if this is a Moon orbital event (apogee, perigee, new/full moon phases)."""
    keywords = [
...
def _is_deep_sky_object(title: str) -> bool:
    """Check if this is a deep-sky object event (Messier, NGC, etc.)."""
    dso_keywords = [
        "messier", "ngc ", "ic ", "pgc ",
        "deep sky", "star cluster", "nebula",
        "galaxy", "supernova remnant",
    ]
    return any(kw in title for kw in dso_keywords)
