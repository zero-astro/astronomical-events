...
def load_config() -> dict:
    ...
    return {
        ...
        "latitude": float(config.get("LATITUDE", "43.1417601")),
        "longitude": float(config.get("LONGITUDE", "-2.9622358")),
...
def run_fetch_pipeline(config: dict) -> dict:
    ...
    try:
        # Step 0: Cleanup past events before fetching new ones
        cleanup_stats = db.cleanup_past_events(keep_days=0)
        if cleanup_stats["events_deleted"] > 0:
            logger.info(
                f"Cleanup: removed {cleanup_stats['events_deleted']} past events "
                f"+ {cleanup_stats['translations_deleted']} translations"
            )
        ...
        for item in raw_items:
            ...
            db.insert_event(
                ...
                visibility_level=classification.visibility_level,
                rich_description_en="",   # Phase 6: populated by scrape phase below
                viewing_info_en="",
                event_details_json="",
            )

        # Scrape thumbnails and rich metadata for events without them (up to 10 per run)
            ...
            try:
                ...
                if page_html:
                    ...
                    if page_data:
                        needs_update = False
                        update_fields = {}
                        if not event.rich_description_en and page_data.rich_description_en:
                            update_fields["rich_description_en"] = page_data.rich_description_en
                            needs_update = True
                        if not event.viewing_info_en and page_data.viewing_info_en:
                            update_fields["viewing_info_en"] = page_data.viewing_info_en
                            needs_update = True
                        if not event.event_details_json and page_data.event_details_json:
                            update_fields["event_details_json"] = page_data.event_details_json
                            needs_update = True
                        if needs_update:
                            db.update_event_metadata(event.news_id, **update_fields)
                            enriched += 1
            except Exception as e:
                logger.warning(f"Scrape failed for {event.title}: {e}")
